...
heroImagePrompt: 'A modern, minimalist technical diagram showing human-AI pair programming workflow with iterative feedback loops on a dark navy blue background (#0a0a0b to #1a1a2e gradient).

SPECIFIC COLLABORATION WORKFLOW ELEMENTS:
LEFT SIDE - Human Developer:
- Stylized developer workstation showing:
  - VS Code editor window (dark theme) displaying code:
    - File: "src/auth/login.ts"
    - Visible code snippet with TypeScript function
  - Terminal panel below showing Claude Code CLI
  - Developer avatar icon (human silhouette) with laptop
- Green glow around the workstation indicating active state
- Text label: "Human: Strategic Direction"

CENTER - Iterative Collaboration Loop (circular flow):
Large circular workflow with 6 numbered stages connected by flowing arrows:

STAGE 1 (Top): "Define Task"
- Human icon
- Card showing: "Issue #47: Implement JWT auth"
- Blue glow

STAGE 2 (Top-Right): "AI Analyzes"
- AI robot icon
- Text: "Reading CLAUDE.md, reviewing existing auth code"
- Purple glow

STAGE 3 (Right): "AI Proposes"
- Code diff preview showing:
  - Green (+) lines: "export function validateToken()"
  - File changes: "3 files modified"
- Purple glow

STAGE 4 (Bottom-Right): "Human Reviews"
- Human icon
- Review interface showing:
  - Comment bubble: "Add expiration check"
  - Thumbs up/down icons
- Green glow

STAGE 5 (Bottom): "AI Refines"
- AI robot icon
- Updated code diff showing:
  - New lines added for expiration validation
  - Green (+) lines with token.exp check
- Purple glow

STAGE 6 (Left): "Validate Together"
- Both icons (human + AI)
- Test results panel:
  - "✓ 12/12 tests passing"
  - "✓ Type check passed"
  - Green checkmarks
- Bright green glow
- Arrow flowing back to STAGE 1 (showing loop can repeat)

RIGHT SIDE - AI Assistant:
- Claude Code interface panel showing:
  - Chat window with conversation:
    - Human: "/dev work #47"
    - Claude: "I'll implement JWT authentication. Let me check..."
    - Claude: "Here's my implementation plan:"
  - Bulleted plan visible
  - Tools panel showing: "Read", "Write", "Grep", "Bash"
- AI robot avatar icon (circuit/neural network design)
- Purple/blue glow around the interface
- Text label: "AI: Tactical Execution"

VISUAL DETAILS:
- Human workstation: realistic VS Code interface, green accents
- AI interface: chat-based UI, purple/blue accents
- Workflow loop: circular arrows flowing clockwise
- Stage cards: rounded rectangles with icons and descriptions
- Code diffs: realistic GitHub diff styling (green +, red -)
- Icons: Human (silhouette), AI (robot/circuit), Review (comment bubble), Tests (checkmark)
- Glowing connection lines between stages
- Stage numbers (1-6) clearly visible
- Alternating colors: green (human actions), purple (AI actions), mixed (collaborative)

COMPOSITION:
- Left: Human workstation (25% width)
- Center: Circular workflow loop (50% width) - FOCAL POINT
- Right: AI interface (25% width)
- Circular flow creates visual balance
- Arrows clearly show directional flow (clockwise)
- Equal visual weight given to human and AI sides

TECHNICAL:
- 16:9 landscape aspect ratio (1792x1024)
- Dark navy blue background (#0a0a0b)
- Human actions: green (#22c55e)
- AI actions: purple (#8b5cf6) and electric blue (#3b82f6)
- Collaborative actions: cyan (#06b6d4)
- Success indicators: bright green (#22c55e)
- Code elements: VS Code dark theme colors
- High contrast for readability
- Monospace font for code, sans-serif for labels
- Professional tech aesthetic (VS Code + modern AI chat interface)
- Circular workflow as primary visual metaphor for iteration'
